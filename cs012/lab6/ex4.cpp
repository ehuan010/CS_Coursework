...
int instinct_main()
{
  ...
  JStar icon_star = JStar(Point(0,2),0.4,YELLOW);
  toolbar.add_icon(&icon_star);
	...
	while (selection!=-1)
	{
      ...
      Point location = win.get_mouse("Choose location");
			...
			/*...FILL HERE ...*/
      JRectangle *rect = new JRectangle(location, DARK_BLUE);
      ...
      Point location = win.get_mouse("Choose location");
			...
			//(you can also use the constructor that has a Point and a Color as parameters if you want colored shapes).
			/*...FILL HERE ...*/
      JSquare *square = new JSquare(location, DARK_GREEN);
    ...
    {
      Point location = win.get_mouse("Choose location");
      JStar *star = new JStar(location, YELLOW);
      star->plot();
  ...
  Message m1 = Message(Point(1,9.8),"Total Area: ");
  Message area = Message(Point(3,9.8), total);
  win << m1 << area;
